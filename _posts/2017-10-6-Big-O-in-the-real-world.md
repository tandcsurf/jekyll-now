...
Until we refine the /nyc bill-scraping API to provide us an array of unique agenda dates, we're going to have to extract the dates from the bills on the client. Since there are hundreds of bills per date, by the time we're done extracting, we'll have a bunch of duplicate dates in our array.
